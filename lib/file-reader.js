...
function contextAwareFileProperties(context, filePath, name, json) {
    ...
    path: {
      get: function () {
        ...
        if (context.current && context.current !== json) {
          ...
          return path.join(relativePath, name + '.html');
        }
        return path.join(toDir, name + '.html');
      }
    },
    root: {
      get: function () {
  ...
  listener.then(function (err, results) {
    if (err) {
      ...
      json.file = Object.create({
        name : name
      }, contextAwareFileProperties(context, filePath, name, json));
      callback(null, json);
    }
  });
