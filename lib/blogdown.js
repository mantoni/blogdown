  ...
  reader.read(source, {}, function (err, data) {
    if (err) {
      ...
      data.items.forEach(function (item) {
        ...
        var relative   = item.file.path.replace(/[^\/]+/g, '..');
        item.file.root = relative.substring(relative.length > 3 ? 3 : 1);
      });
      try {
        processor.process(data.items, source, options);
        var lists = list.createAll(data.items, options.lists);
        var files = renderer.render(data.items, lists, data.partials);
